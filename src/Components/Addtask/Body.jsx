...
const Body = () => {
  ...
  const addToPortfolio = () => {
    ...
    const existingGoalIndex = existingTodos.findIndex(item => item.goalName.toLowerCase() === goal.trim().toLowerCase())
    ...
    <>
      ...
      <div className="container">
        <div className="inp-cont">
          <input value={goal} onChange={e => setGoal(e.target.value)} placeholder="Enter your goal (e.g., DSA, Web Development)" />
        ...
        <div className="btn-cont">
          <button onClick={generateTasks} disabled={loading}>{loading ? 'Loading...' : 'Get Tasks'}</button>
          <button onClick={addToPortfolio} disabled={loading || tasks.length === 0}>{loading ? 'Processing...' : 'Add to Portfolio'}</button>
        </div>
        <div className="tasks">
          {tasks.map((t, i) => (
